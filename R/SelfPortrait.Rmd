...
```
### source[aschincon](https://github.com/aschinchon/clustering-frankenstein/blob/master/clust_frank.R)
```{r}
library(imager)
library(tidyverse)
library(purrr)
...
```{r loading of the photograph}
file = "C:/Users/inkyscope/Documents/projects/image/inky.jpg"
```


```{r}
# Load, convert to grayscale, filter image (to convert it to bw) and sample
inky <- load.image(file) %>% 
  grayscale() %>% 
  threshold("45%") %>% 
  as.cimg() %>% 
  as.data.frame() 

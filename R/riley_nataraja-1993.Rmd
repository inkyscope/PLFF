...
```
###Source[aRist/remakes/reley by @geokaramanis] (https://github.com/gkaramanis/aRtist/blob/main/remakes/riley/nataraja-1993.R)
```{r}
library(tidyverse)
```
...
ggplot(nataraja) +
  geom_polygon(aes(x = x, y = y, fill = factor(col), 
                   group = interaction(c, r)), color = NA) +

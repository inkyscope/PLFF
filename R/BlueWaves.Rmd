...
### Source[aRt by gkaramanis] (https://github.com/gkaramanis/aRt/blob/master/flow/flow.R)
```{r}
library(tidyverse)
library(ambient)
library(here)
...
ggplot(dat_df) +
  geom_segment(aes(x = x_start, y = y_start,
               ...
               alpha = curve_alpha,
               lineend = "butt", 
               linejoin = "round") + # change those for different effects: lineend = c('round', 'butt', 'square'), linejoin = c('round', 'mitre', 'bevel')
  coord_fixed(xlim = c(limit * 1.5, nx - limit * 1.5), 
              ylim = c(limit * 1.5, ny - limit * 1.5)) + # "crop" to fill the frame
  theme_void() +
  theme(
    legend.position = "none",
    plot.background = element_rect(fill = "#1b1f2b")  # "#ffc50c", #006766"
  ) 

  ggsave(here::here("figures", "BlueWaves.png"), dpi = 320, width = 7, height = 7)
```
